...
import { mimeFromExtension, readUriAsArrayBuffer } from '../../src/lib/storage'
    ...
    <View style={styles.card}>
      <View style={styles.cardTopo}>
        ...
        <View style={styles.cardTextos}>
          ...
          {exame.local ? <Text style={styles.cardSubtitulo}>{exame.local}</Text> : null}
        </View>
...
export default function Exames() {
    ...
    useCallback(() => {
      async function init() {
        ...
        const { data: perfil } = await supabase
          .from('perfis').select('nome, foto_url').eq('id', user.id).single()
        if (perfil) {
          if (perfil.foto_url) {
  ...
  function abrirVisualizador(url: string, isImage: boolean) {
    setViewerUrl(url); setViewerIsImage(isImage); setViewerVisivel(true)
  }
  ...
  async function excluir() {
    ...
    if (exa) await salvarHistorico(usuarioId!, `Exame ${exa.nome} foi removido`)
    setModalExcluir(false); setExcluirId(null)
    await buscar()
    await reagendarTodasNotificacoes(usuarioId!).catch(console.error)
    setModalSucesso({
      visivel: true, titulo: 'Exame excluído!',
  ...
  return (
    <SafeAreaView style={[styles.safe, hideList === 'true' && { backgroundColor: 'transparent' }]} edges={['top']}>
      {hideList !== 'true' && (
        <>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
            ...
            <View style={styles.filtrosRow}>
              {(['proximos', 'realizados', 'todos'] as Filtro[]).map((f) => (
                <TouchableOpacity
                  key={f} onPress={() => setFiltro(f)} activeOpacity={0.8}
                  style={[styles.chip, filtro === f && styles.chipAtivo]}
                >
                  {filtro === f ? (
                    <LinearGradient colors={['#6B49AD', '#481D94']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.chipGradient}>
          ...
          <TouchableOpacity onPress={() => abrirModal()} activeOpacity={0.85} style={styles.fab}>
            <LinearGradient colors={['#6B49AD', '#481D94']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.fabGradient}>
          ...
          <Animated.View style={[styles.modalCard, { transform: [{ translateY: slideAnim }] }]}>
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitulo}>{editando ? 'Editar exame' : 'Novo exame'}</Text>
              ...
              <Campo
                label="NOME DO EXAME" value={nome}
                onChangeText={(v) => { setNome(v); setErros(p => ({ ...p, nome: false })) }}
                placeholder="Ex: Hemograma completo" erro={erros.nome}
              />

              <View style={styles.duasColunas}>
                <View style={[styles.coluna, { flex: 3 }]}>
                  <Campo
                    label="DATA REALIZAÇÃO" value={dataRealizacao}
                    onChangeText={(t) => { setDataRealizacao(mascaraData(t)); setErros(p => ({ ...p, dataRealizacao: false })) }}
                    placeholder="DD/MM/AAAA" keyboardType="numeric" erro={erros.dataRealizacao}
                  />
                </View>
                <View style={[styles.coluna, { flex: 2 }]}>
                  <Campo
                    ...
                    placeholder="HH:MM" keyboardType="numeric" erro={erros.horario}
                  />
                </View>
              </View>

              <Campo
                label="DATA RESULTADO" value={dataResultado}
                onChangeText={(t) => setDataResultado(mascaraData(t))}
                placeholder="DD/MM/AAAA" keyboardType="numeric" opcional
              />

              <Campo label="LOCAL" value={local} onChangeText={setLocal} placeholder="Ex: Laboratório Central" opcional />
                ...
                {arquivoUrlAtual ? (
                  <View style={styles.previewContainer}>
                    ...
                    <View style={styles.previewDetalhes}>
                      ...
                      >
                        <Feather
                          name={isImageUrl(arquivoUrlAtual) ? 'image' : 'file-text'}
                          size={13}
                          color="#6B49AD"
                        />
                        <Text style={styles.verArquivoBtnTexto}>
              ...
              <TouchableOpacity onPress={salvar} disabled={carregando} activeOpacity={0.85} style={styles.botaoSalvarWrapper}>
                <LinearGradient
                  colors={['#6B49AD', '#481D94']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
      ...
      <ModalAlerta
        visivel={modalAlerta.visivel} titulo={modalAlerta.titulo} mensagem={modalAlerta.mensagem}
        onFechar={() => setModalAlerta({ visivel: false, titulo: '', mensagem: '' })}
      />
      <ModalAlerta
        visivel={modalSucesso.visivel} titulo={modalSucesso.titulo} mensagem={modalSucesso.mensagem}
        onFechar={() => {
...
const styles = StyleSheet.create({
  ...
  filtrosRow: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginTop: 12, gap: 8 },
  chip: { flex: 1, borderRadius: 999, overflow: 'hidden', borderWidth: 1.5, borderColor: '#6B49AD' },
  chipAtivo: {
    borderColor: 'transparent', shadowColor: '#481D94',
  ...
  fab: {
    position: 'absolute', bottom: 28, right: 24, borderRadius: 999,
  ...
  card: { backgroundColor: '#FAFAFE', borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#EDE8FA' },
  ...
  badgeProxima: {
    ...
    backgroundColor: '#E6F1FB', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4,
  },
  badgeProximaTexto: { fontSize: 11, fontWeight: '700', color: '#185FA5' },
  badgeRealizada: {
    flexDirection: 'row', alignItems: 'center', gap: 4, alignSelf: 'flex-start',
    backgroundColor: '#EDE8FA', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4,
  },
  badgeRealizadaTexto: { fontSize: 11, fontWeight: '700', color: '#6B49AD' },

  verMaisBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 4, marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#F0EAFF',
  },
  ...
  btnArquivoCard: {
    ...
    paddingHorizontal: 12, paddingVertical: 10, borderWidth: 1, borderColor: '#E2D9F3',
  },
  btnArquivoCardIcone: { width: 30, height: 30, borderRadius: 8, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' },
  ...
  previewImage: { width: 60, height: 60, borderRadius: 12, borderWidth: 1, borderColor: '#E2D9F3' },
  pdfIconeBox: { width: 60, height: 60, borderRadius: 12, backgroundColor: '#FFEBEE', justifyContent: 'center', alignItems: 'center' },
  ...
  verArquivoBtnTexto: { fontSize: 12, fontWeight: '700', color: '#6B49AD' },
  ...
  btnExcluirConfirmar: { width: '100%', backgroundColor: '#dc2626', borderRadius: 50, paddingVertical: 18, alignItems: 'center' },
  ...
  viewerHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14 },
  viewerVoltarBtn: {
    flexDirection: 'row', alignItems: 'center',
  ...
  viewerFechar: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(107, 73, 173, 0.4)', justifyContent: 'center', alignItems: 'center' },
  viewerTitulo: { flex: 1, fontSize: 16, fontWeight: '700', color: '#fff', textAlign: 'center', marginHorizontal: 8 },
  viewerAbrirExterno: { width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(107, 73, 173, 0.4)', justifyContent: 'center', alignItems: 'center' },
  ...
  viewerPdfIcone: { width: 120, height: 120, borderRadius: 28, backgroundColor: 'rgba(255,255,255,0.1)', justifyContent: 'center', alignItems: 'center' },
  viewerPdfLabel: { fontSize: 11, fontWeight: '800', color: '#dc2626', marginTop: 6, letterSpacing: 2 },
  viewerPdfTexto: { fontSize: 15, color: 'rgba(255,255,255,0.7)', textAlign: 'center', lineHeight: 22 },
  viewerPdfBotao: { borderRadius: 999, shadowColor: '#481D94', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 6, marginTop: 8 },
  viewerPdfBotaoGradient: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 999, paddingHorizontal: 28, paddingVertical: 16 },
